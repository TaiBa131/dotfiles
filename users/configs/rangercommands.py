...
| (_| (_) | | | | | | | | | | | (_| | | | | (_| \__ \_| |_) | |_| |
 \___\___/|_| |_| |_|_| |_| |_|\__,_|_| |_|\__,_|___(_) .__/ \__, |
                                                      |_|    |___/
My own commands
"""
...
class empty(Command):
    ...
    Be careful with this, it could delete the whole home dir if not careful
    """
...
class directorysize(Command):
    ...
    Prints the storage space taken by the current dir
    """

    def execute(self):
        result=os.popen("du -sh ./ | awk '{print $1}'")
        dirsize=result.read()
        self.fm.notify(dirsize)
...
class fzf_search(Command):
    ...
    With a dirs argument select only directories.
    With a local argument select only local files.
